...
def ellipse_strip(r, tau, y, hjd_central, i, phi, star, width):
    ...
    nx = np.ceil(2*(np.amax(r) + 1.25 * width) / dt)
    yc = (ny - 1) / 2.
    xc = (nx - 1) / 2.
    ...
    tau_disk_convolved = convolve(tau_disk, star, mode='valid')

    # pick out central rows from convolution and the actual xvals
    yc = int(yc)
    x_tdc = agr[1, yc, yc:-yc] + hjd_central
    tdc = 1-tau_disk_convolved[0]
    td = 1-tau_disk[yc, yc:-yc]
    g = np.abs(np.sin(grad[yc, yc:-yc]))

    return(tau_disk, tau_disk_convolved, np.vstack((x_tdc, td, tdc, g)))
...
if __name__ == '__main__':
    ...
    a_test = np.mgrid[:1, :401].astype(np.float)
    xx = np.arange(401)
    a_test[0] += y2
    a_test[1] -= xc
